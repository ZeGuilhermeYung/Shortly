...
function urlSchemaValidation (validator) {
    if (!urlSchemas[validator]) {
        return `${validator} does not exist`;
    }

    return async function (req, res, next) {
        const validation = urlSchemas[validator].validate(req.body, { abortEarly: false });
        if (validation.error) {
            const errors = validation.error.details.map(error => error.message);
            res.status(422).send({ message: errors });
            return;
...
export { schemaValidation, urlSchemaValidation };